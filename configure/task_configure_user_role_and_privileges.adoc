...
+
Consultez l'article de la base de connaissances  https://kb.netapp.com/mgmt/OTV/Virtual_Storage_Console/Virtual_Storage_Console%3A_How_to_retrieve_the_JSON_file_to_configure_user_roles_and_privileges["Virtual Storage Console : comment récupérer le fichier JSON pour configurer les rôles et privilèges des utilisateurs"^] pour obtenir des instructions sur la façon de télécharger le fichier ONTAP Privileges à partir de Web-CLI.
