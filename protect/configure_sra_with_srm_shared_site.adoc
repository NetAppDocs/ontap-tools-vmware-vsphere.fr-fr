...
[role="lead"]
ONTAP tools for VMware vSphere prend en charge la configuration du site de récupération partagé SRM de VMware. Pour plus d'informations, consultez :  https://techdocs.broadcom.com/us/en/vmware-cis/live-recovery/site-recovery-manager/8-8/site-recovery-manager-installation-and-configuration-8-8.html["Site Recovery Manager dans une configuration de site de reprise partagé"^]. Les instances du serveur Site Recovery Manager sur le site de récupération se connectent aux mêmes instances de serveur vCenter.
...
L' https://kb.netapp.com/mgmt/OTV/SRA/Storage_Replication_Adapter%3A_How_to_configure_SRA_in_a_SRM_Shared_Recovery_Site["Comment configurer SRA sur un site de reprise partagé SRM"^]article de la base de connaissances détaille la procédure de configuration de SRA pour prendre en charge la configuration du site de récupération partagé SRM.
