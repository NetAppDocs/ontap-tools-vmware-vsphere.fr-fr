...
Si le système de stockage est ajouté avec un utilisateur personnalisé créé dans le périmètre du cluster à l'aide du fichier json et que vous souhaitez effectuer une mise à niveau vers la version 9.12, exécutez les commandes ci-dessous sur l'interface de ligne de commande ONTAP avant la mise à niveau pour activer la communication basée sur le certificat entre les outils ONTAP pour VMware vSphere et ONTAP :
