...
L'article suivant décrit la procédure d'ajout de stockage aux ONTAP tools lorsque VCF est activé, https://kb.netapp.com/mgmt/OTV/SRA/Storage_Replication_Adapter%3A_How_to_configure_SRA_in_a_SRM_Shared_Recovery_Site["Ajoutez du stockage aux outils ONTAP à partir de l'interface swagger"^].
